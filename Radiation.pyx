...
cdef class Radiation:
    ...
    cdef update_RRTM(self, ReadProfiles.ReadProfiles pf):
        ...
        for k in xrange(nz):
            ...
            cldfr_raw[k] = pf.cloud_fraction[k]
        ...
        with nogil:
            for k in xrange(nz_full):
                ...
                if self.uniform_reliq:
                    reliq_raw[k] = 14.0*cldfr_raw[k]
                else:
                    reliq_raw[k] = ((3.0*self.p_full[k]/Rd/tlay_in[k]*rl_full[k]/
                                        fmax(cldfr_raw[k],1.0e-6))/(4.0*pi*1.0e3*100.0))**(1.0/3.0)
        ...
        c_mcica_subcol_lw (&iplon, &ncol, &nlay, &icld, &seedlw, &irng,
            &play_in[0], &cldfr_raw[0], &cicewp_raw[0], &cliqwp_raw[0],
            &reice_raw[0], &reliq_raw[0], &taucld_lw_raw[0,0],
            &cldfr_lw_in[0,0], &cicewp_lw_in[0,0], &cliqwp_lw_in[0,0],
            &reice_lw_in[0], &reliq_lw_in[0], &taucld_lw_in[0,0])

        # plt.figure(6)
        # plt.subplot(121)
        # plt.plot(cliqwp_raw, self.p_full, 'k')
        # plt.plot(np.mean(cliqwp_lw_in, axis=0), self.p_full, 'r')
        # plt.xlabel('reliq_in')
        # plt.subplot(122)
        # plt.plot(cicewp_raw, self.p_full,'k')
        # plt.plot(np.mean(cicewp_lw_in, axis=0), self.p_full, 'c')
        # plt.xlabel('reice_in')
        # plt.figure(7)
        # # plt.subplot(211)
        # plt.contourf(cldfr_lw_in)
        # plt.colorbar()
        # # plt.subplot(212)
        # # plt.contourf(cicewp_lw_in)
        # # plt.colorbar()
        # plt.show()
