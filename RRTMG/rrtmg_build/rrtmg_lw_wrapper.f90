...
subroutine c_mcica_subcol_lw(iplon, ncol, nlay, icld, permuteseed, irng, play, &
                       cldfrac, ciwp, clwp, rei, rel, tauc, cldfmcl, &
                       ciwpmcl, clwpmcl, reicmcl, relqmcl, taucmcl) bind(c)

! ----- Input -----
! Control
      integer(c_int), intent(in) :: iplon           ! column/longitude index
...
! Atmosphere/clouds - cldprop
      real(c_double), intent(in) :: cldfrac(ncol,nlay)       ! layer cloud fraction
                                                      !    Dimensions:
                                                      !    (ncol,nlay)
      real(c_double), intent(in) :: tauc(ngptlw,ncol,nlay)        ! in-cloud optical depth
                                                      !    Dimensions:
                                                      !    (nbndlw,ncol,nlay)
!      real(c_double), intent(in) :: ssac(ngptlw,ncol,nlay)       ! in-cloud single scattering albedo
                                                      !    Dimensions:
                                                      !    (nbndlw,ncol,nlay)
!      real(c_double), intent(in) :: asmc(ngptlw,ncol,nlay)       ! in-cloud asymmetry parameter
                                                      !    Dimensions:
                                                      !    (nbndlw,ncol,nlay)
...
! Atmosphere/clouds - cldprmc [mcica]
      real(c_double), intent(out) :: cldfmcl(ngptlw,ncol,nlay)    ! cloud fraction [mcica]
                                                      !    Dimensions:
                                                      !    (ngptlw,ncol,nlay)
      real(c_double), intent(out) :: ciwpmcl(ngptlw,ncol,nlay)    ! in-cloud ice water path [mcica]
                                                      !    Dimensions:
                                                      !    (ngptlw,ncol,nlay)
      real(c_double), intent(out) :: clwpmcl(ngptlw,ncol,nlay)    ! in-cloud liquid water path [mcica]
                                                      !    Dimensions:
                                                      !    (ngptlw,ncol,nlay)
      ...
      real(c_double), intent(out) :: reicmcl(ncol,nlay)      ! ice partcle size (microns)
                                                      !    Dimensions:
                                                      !    (ncol,nlay)
      real(c_double), intent(out) :: taucmcl(ngptlw,ncol,nlay)    ! in-cloud optical depth [mcica]
                                                      !    Dimensions:
                                                      !    (ngptlw,ncol,nlay)
!      real(c_double), intent(out) :: ssacmcl(ngptlw,ncol,nlay)   ! in-cloud single scattering albedo [mcica]
                                                      !    Dimensions:
                                                      !    (ngptlw,ncol,nlay)
!      real(c_double), intent(out) :: asmcmcl(ngptlw,ncol,nlay)   ! in-cloud asymmetry parameter [mcica]
                                                      !    Dimensions:
                                                      !    (ngptlw,ncol,nlay)
...
end subroutine c_mcica_subcol_lw
...
subroutine c_rrtmg_lw &
      ...
      real(c_double), intent(in) :: cldfr(ngptlw,ncol,nlay)         ! Cloud fraction
                                                      !    Dimensions: (ncol,nlay)
      real(c_double), intent(in) :: cicewp(ngptlw,ncol,nlay)        ! Cloud ice water path (g/m2)
                                                      !    Dimensions: (ncol,nlay)
      real(c_double), intent(in) :: cliqwp(ngptlw,ncol,nlay)        ! Cloud liquid water path (g/m2)
                                                      !    Dimensions: (ncol,nlay)
      ...
      real(c_double), intent(in) :: reliq(ncol,nlay)         ! Cloud water drop effective radius (microns)
                                                      !    Dimensions: (ncol,nlay)
      real(c_double), intent(in) :: taucld(ngptlw,ncol,nlay)      ! In-cloud optical depth
...
! ----- Optional Output -----
      real(c_double), intent(out), optional :: duflx_dt(ncol,nlay+1)     
                                                      ! change in upward longwave flux (w/m2/k)
                                                      ! with respect to surface temperature
                                                      !    Dimensions: (ncol,nlay+1)
      real(c_double), intent(out), optional :: duflxc_dt(ncol,nlay+1)    
